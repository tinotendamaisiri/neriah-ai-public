        ...
        return Promise.parallel {
            ...
            synchronized(initLock) {
                ...
                try {
                    ...
                    //
                    // Deliberately omit maxNumTokens. The Kotlin SDK passes
                    // it through to the engine's max_num_tokens setting, but
                    // that value is the KV-cache *context length* (in
                    // tokens), NOT a generation cap. Hard-coding it to e.g.
                    // 1024 caps the cache slot below what Gemma 4 E2B's
                    // compiled prefill graph expects, and the magic-number
                    // remap then wires a context-dim of 1024 into a graph
                    // compiled for a longer context. The first prefill
                    // invocation then trips
                    //   tflite/kernels/dynamic_update_slice.cc:70
                    //     SizeOfDimension(update, i) <= SizeOfDimension(operand, i)
                    // because the prefill chunk is bigger than the (capped)
                    // cache slot.
                    //
                    // Leaving it unset → null → JNI receives -1 → JNI's
                    // `if (max_num_tokens > 0)` guard skips SetMaxNumTokens
                    // → engine_settings.cc default heuristic kicks in,
                    // reading the model's compiled context magic. Same
                    // approach iOS already takes (see HybridLiteRTLM.cpp).
                    //
                    // Output token cap is supposed to live on the session
                    // (set_max_output_tokens), but the Kotlin SessionConfig
                    // in this AAR doesn't expose that field — so for now
                    // we rely on the model's natural <end_of_turn> token
                    // and the SDK's internal stop conditions.
